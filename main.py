...
def sql_table_controll( db_name,
    ...
    order_search_options =   ['', f'ORDER BY {column} DESC']
...
def Menu_printer(opts,title = "", shape = "=", space_line = True, head_steps = 0,
    ...
    while lop :
        ...
        if space_line :
            for key, value in opts.items() :print(f"{key} -> {value}")
            print(shape * head_steps, end="\n")     
        else : 
            for key, value in opts.items() :print(f"{key} -> {value}", end="")
